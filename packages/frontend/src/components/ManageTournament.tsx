...
const Scoreboard: React.FC = () => {
  ...
  useEffect(() => {
    if (selectedTournament !== null) {
      ...
      const fetchPlayers = async () => {
        try {
          const response = await axios.get(
            `http://localhost:3001/api/tournaments/${selectedTournament}/players`,
          );
  ...
  const checkForGameWinner = (updatedGameScores: {
    [game: number]: { [key: string]: number };
  }) => {
    ...
    if (player1Score >= 21 && player1Score >= player2Score + 2) {
      ...
      handleGameWin(
        player1Score === 30 ? selectedPlayers[0] : selectedPlayers[1],
      );
    }
  };
  ...
  const handleGameWin = (winner: string) => {
    setGameWinners((prevWinners) => {
      ...
      const winnerCounts = {
        [selectedPlayers[0]]: Object.values(newWinners).filter(
          (w) => w === selectedPlayers[0],
        ).length,
        [selectedPlayers[1]]: Object.values(newWinners).filter(
          (w) => w === selectedPlayers[1],
        ).length,
      };
      ...
      if (winnerCounts[winner] === 2) {
        ...
        alert(
          `${players.find((p) => p.bsa_bwf_number === winner)?.name} wins the match!`,
        );
      } else {
  ...
  const handleStartMatch = async () => {
    try {
      const response = await axios.post(
        "http://localhost:3001/api/tournaments/:tournament_id/matches",
        {
  ...
  useEffect(() => {
    if (currentMatchId !== null) {
      const fetchScores = async () => {
        try {
          const response = await axios.get(
            `http://localhost:3001/api/tournaments/:tournament_id/matches/${currentMatchId}/scores`,
          );
    ...
    <div>
        ...
        <div>
          ...
          <div>
            ...
            >
              ...
              {players.map((player) => (
                <option
                  key={player.bsa_bwf_number}
                  value={player.bsa_bwf_number}
                >
      ...
      {selectedPlayers.length > 0 && (
        <div>
          {[1, 2, 3].map((game) => (
            <div
              key={game}
              style={{ color: currentGame === game ? "black" : "grey" }}
            >
              <h3>Game {game} Scores</h3>
              {selectedPlayers.map((player) => (
                <div key={player}>
                  <p>
                    {players.find((p) => p.bsa_bwf_number === player)?.name}{" "}
                    {players.find((p) => p.bsa_bwf_number === player)?.surname}{" "}
                    {servingPlayer === player ? "*" : ""}
                    <button
                      onClick={() => handleScoreChange(player, true)}
                      disabled={game !== currentGame || !!gameWinners[game]}
                    >
                      +
                    </button>
                    <span>{(gameScores[game]?.[player] ?? 0).toString()}</span>
                    <button
                      onClick={() => handleScoreChange(player, false)}
                      disabled={game !== currentGame || !!gameWinners[game]}
                    >
                      -
                    </button>
              ...
              {gameWinners[game] && (
                <div>
                  <h3>
                    ...
                    {
                      players.find(
                        (p) => p.bsa_bwf_number === gameWinners[game],
                      )?.name
                    }
                  </h3>
                </div>
      ...
      {matchWinner && (
        <div>
          <h2>
            Match Winner:{" "}
            {players.find((p) => p.bsa_bwf_number === matchWinner)?.name}
          </h2>
        </div>
      )}
    </div>
